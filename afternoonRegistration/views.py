...
def add_supplier(request):
    ...
    if request.method == "POST":
        ...
        supplier = Supplier(sup_name=name, sup_email=email, sup_phone=phone,
                            sup_product=product)
        supplier.save()
        # Redirect back to add_supplier page with a success message
...
def view_suppliers(request):
    ...
    return render(request, 'view-suppliers.html', context)
...
def update_supplier(request, id):
    ...
    supplier = Supplier.objects.get(id=id)
    return render(request, 'update-One.html', {'supplier': supplier})
...
# Prepare a function to generate transaction with auth token
